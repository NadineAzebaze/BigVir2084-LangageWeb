...
YES = 'yes', 'y', 'ok'
NO = 'no', 'n'
path = {'xslt_parser': './xslt_parser.py','xsd_validator': './xsd_validator.py',
​	'xml': '../XML/*','xml_doc': '../XML/BigVir2084.xml',
	'xml_schema': '../XML/BigVir2084.xsd','xml_transformations': '../XML/transformations/*',
	'output': '../XML/transfo_output/*',​'json': '../JSON/*',
        'json_doc': '../JSON/BigVir2084.json','json_schema': '../JSON/BigVir2084Schema.json'
	}
​
​def EXIT():
...
​
	for t, output_file_name in zip(trees, output_files_names):
		...
		output = etree.tostring(transformator(xml_doc), pretty_print = True)		
                if output_folder == None:
			result += output,
		else:
			...
			with open(output_full_path, 'wb') as file:
				file.write(output)
			print(f'File "{output_full_path}" has been overriden')
​
​
	if output_folder == None:
		return result
...
​def install_pip():
	...
	if osCoreName == 'Linux':
		pip_install_commands = 'sudo apt install pip',
